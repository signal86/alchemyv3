...
impl ASTNode {
    pub fn new(node: ASTNodeT, line: u128) -> Self {
        Self { node, line }
    }
}
...
pub fn parse_Node(lexer: &mut Lexer) -> Result<ASTNode, Error> {
    ...
    match lexer.curr_token.t {
                ...
                match lexer.curr_token.t {
                    TokenType::Keyword => {
                        if lexer.curr_token.lexeme.as_str() == "component" {
                            Ok(ASTNode::new(
                                ASTNodeT::ComponentDefinition(parse_ComponentDefinition(lexer)?),
                                source_line,
                            ))
                        } else {
                            Err(Error::new(
                    ...
                    TokenType::Identifier => Ok(ASTNode::new(
                        ASTNodeT::ComponentInstance(parse_ComponentInstance(lexer)?),
                        source_line,
                    )),
                    _ => Err(Error::new(
                        ErrorKind::Other,
        ...
        TokenType::Identifier => Ok(ASTNode::new(
            ASTNodeT::Assignment(parse_Assignment(lexer)?),
            source_line,
        )),
